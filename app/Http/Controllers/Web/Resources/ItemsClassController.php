...
class ItemsClassController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'required',
            'alpha_num',
                Rule::unique('items_classes_resources')->where(function ($query) use ($request) {
    ...
    public function edit(Request $request)
    {
        ...
        $id = auth('sanctum')->id();
        $item = ItemsClassesResource::query()->findOrFail($request->Item_id);
        ...
        if ($validator->fails()) {
            return mainResponse(false, '' , null, 203, 'items',$validator);
                ...
                $item = ItemsClassesResource::findOrFail($request->Item_id);
                $item->company_id = company_auth_id();
                $item->category_id = $request->category_id;
                $item->code = $request->code;
                $item->type_id = ItemsTypes::where('name',$request->type)->pluck('id')->first();
    ...
    public function getAll(Request $request)
    {
         ...
         $id = auth('sanctum')->id();
            ...
            $items = ItemsClassesResource::with('type','category')->where('company_id', company_auth_id());
    ...
    {
        $items = ItemsClassesResource::with('type','category')->where('id',$request->ID)->first();
        $message ="success return";
        return mainResponse(true, $message , $items, 200, 'items',''); 
    }
    ...
    {
        if(ItemsClassesResource::where('id',$id)->delete()){
            $message ="success return";
