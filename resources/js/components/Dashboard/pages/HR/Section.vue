            ...
            <tr v-for="(item, index) in items" :key="item.id">
              ...
              <td class="align-items-center" style="margin-top: 15px;">
                <div class="d-flex flex-column">
                  <a href="#" class="text-gray-800 text-hover-primary mb-1">{{item.department.name}}</a>
                </div>
              </td>
  ...
  components: {
    ...
    methods: {
        ...
        async fetchItems(page,query = '') {
          ...
          this.isLoading = true;
            await axios.get('Sections/getAllItems', {
                params: {
        ...
        async getItemById(){
            ...
            axios.get('Sections/getById', { params: { ID: this.ItemID } }).then(response => {
                if(response.data){
        ...
        deleteItem(id) {
          swal.fire({
            ...
            if (result.isConfirmed) {
              // إذا تم تأكيد الحذف
              axios.delete(`Sections/deleteItem/${id}`)
                .then(() => {
